...
import musicFinder from '../../assets/images/musicFinder.png'; // Assuming you have a musicFinder image
...
export default function SearchScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-black px-4">
      {/* Search Bar */}
      <View className="flex-row items-center bg-white p-1 rounded-lg mt-4">
        <Ionicons name="search-outline" size={20} color="#1C1F20" />
        <TextInput
          placeholder="Search songs, albums..."
          placeholderTextColor="gray"
          className="flex-1 text-white ml-2 font-LRegular text-[18px]"
          value={search}
      ...
      {/* MusicFinder Banner */}
      <TouchableOpacity className="flex-row items-center bg-[#3A3A3D] p-2 rounded-lg mt-3">
        <Image
          source={musicFinder}
        ...
        />
        <View className="ml-3">
          <Text className="text-blue-400 font-LBold text-[18px]">MusicFinder</Text>
          <Text className="text-gray-400 font-LRegular text-[13px]">Determine which song is currently playing</Text>
        </View>
      </TouchableOpacity>
      ...
      {search === '' && recentSearches.length > 0 && (
        <View className="mt-5">
          ...
          {recentSearches.map((title, index) => (
            <Text key={index} className="text-white mb-2">• {title}</Text>
          ))}
        </View>
      )}
